...
public class MedicalRecordController {
	...
	public ResponseEntity<List<MedicalRecord>> getAllRecords() {
		return ResponseEntity.status(HttpStatus.CREATED).body(medicalRecordService.getAllMedicalRecords());
	}

	@GetMapping("/get/{recordId}")
	public ResponseEntity<MedicalRecord> getRecordById(@PathVariable int recordId) {
		return ResponseEntity.status(HttpStatus.CREATED).body(medicalRecordService.getMedicalRecordById(recordId));
	}

	@GetMapping("/get/patient")
	public ResponseEntity<List<MedicalRecord>> getRecordsByPatient(Principal principal) {
		String username = principal.getName();
		return ResponseEntity.status(HttpStatus.CREATED).body(medicalRecordService.getMedicalRecordsByPatientUsername(username));
	}
	...
	public ResponseEntity<List<MedicalRecord>> getRecordsByPatientId(@RequestParam(defaultValue = "0") int page,
		...
		return ResponseEntity.status(HttpStatus.CREATED).body(medicalRecordService.getMedicalRecordsByPatientId(id,page,size));
	}

	@GetMapping("/get/doctor")
	public ResponseEntity<List<MedicalRecord>> getRecordsByDoctor(Principal principal) {
		String username = principal.getName();
		return ResponseEntity.status(HttpStatus.CREATED).body(medicalRecordService.getMedicalRecordsByDoctorUsername(username));
	}
	...
	public ResponseEntity<List<MedicalRecord>> getRecordsByRecordDate(@RequestParam("date") LocalDate recordDate) {
		return ResponseEntity.status(HttpStatus.CREATED).body(medicalRecordService.getMedicalRecordsByRecordDate(recordDate));
	}
}
